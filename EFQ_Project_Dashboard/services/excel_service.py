...
def get_incident_by_id(incident_id: str) -> Optional[Dict[str, Any]]:
    """Get incident by ID."""
    incidents = get_incidents()
    matching = incidents[incidents["IncidentID"] == incident_id]
    if matching.empty:
        return None
    return matching.iloc[0].to_dict()


def get_activities_by_incident(incident_id: str) -> pd.DataFrame:
    """Get all activities for a specific incident."""
    activities = get_activities()
    return activities[activities["IncidentID"] == incident_id]